...
const tools = contentStore.toolsList
...
const filteredTools = computed(() => {
  if (!selectedLevel.value) return tools
  return tools.filter(t => t.levels.includes(selectedLevel.value))
})
...
<template>
  <div class="tools-view">
    ...
    <div class="categories-list">
      <div v-for="(catTools, category) in categories" :key="category"
           class="category-section custom-card no-hover stagger-item animate__animated animate__fadeInUp">
        <h2>
        ...
        <div class="tools-grid">
          <div v-for="tool in catTools" :key="tool.name" class="tool-item">
            <div class="tool-icon-wrap">
              <i :class="tool.icon" style="color: var(--text-secondary)"></i>
            </div>
            <div class="tool-info">
              <h4>{{ tool.name }}</h4>
              <div class="tool-levels">
                <span
                  v-for="lvl in tool.levels"
                  :key="lvl"
                  class="level-dot"
                  :style="{ background: levelColors[lvl] }"
                  v-tooltip.top="`المستوى ${lvl}`"
                >
                  {{ lvl }}
                </span>
              </div>
