...
const assessmentTools = contentStore.assessmentTools
</script>
    ...
    <TabView>
      <TabPanel v-for="level in levels" :key="level.id" :header="level.name">
        <div class="assessment-content" :style="{ '--lc': level.color }">
          ...
          <div v-for="(questions, category) in assessmentTools[`level${level.id}`]" :key="category" class="assessment-category">
            <h3>
              <Tag :style="{ background: level.color + '20', color: level.color }" :value="
                category === 'phonetic' ? 'الوعي الصوتي' :
                category === 'visual' ? 'الوعي البصري' :
                category === 'writing' ? 'الكتابة' :
                category === 'reading' ? 'القراءة' :
                category === 'linguistics' ? 'اللغويات' : category
              " />
            </h3>
            <div class="checklist">
              <div v-for="(question, idx) in questions" :key="idx" class="checklist-item stagger-item" :style="{ animationDelay: `${idx * 0.05}s` }">
